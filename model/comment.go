...
func SQLCommentList(db *sql.DB, cntDB *youdb.DB, topic_id, start uint64, limit, tz int) CommentPageInfo {
	...
	rows, err := db.Query(
		"SELECT id, user_id, topic_id, content, created_at FROM  reply WHERE topic_id = ? And id > ? ORDER BY id limit ?",
		topic_id,
		start,
		limit,
	)
	...
	for rows.Next() {
		...
		item.ContentFmt = template.HTML(util.ContentFmt(cntDB, item.Content))
